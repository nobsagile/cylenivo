...
export default function ForecastPage() {
  ...
  // Histogram — highlight bars past p50/p85/p95
  const p50 = result?.p50 ?? 0
  const p85 = result?.p85 ?? 0
  const p95 = result?.p95 ?? 0

  function barColor(bucket: number) {
    if (bucket <= p50) return '#0d9488'   // teal
    if (bucket <= p85) return '#7c3aed'   // violet
    if (bucket <= p95) return '#f43f5e'   // rose
    return '#e5e7eb'                       // gray
  }
        ...
        <>
          ...
          <div className="flex gap-3">
            <PercentileResult
              ...
              value={result.p50}
              unit={mode === 'how_many'
                ? t('forecast.ticketsInN', { n: value })
                : t('forecast.weeksForN', { n: value })}
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-5">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={result.histogram} barCategoryGap="10%">
                ...
                />
                <ReferenceLine x={result.p50} stroke="#0d9488" strokeDasharray="3 3" label={{ value: 'p50', fill: '#0d9488', fontSize: 10 }} />
                <ReferenceLine x={result.p85} stroke="#7c3aed" strokeDasharray="3 3" label={{ value: 'p85', fill: '#7c3aed', fontSize: 10 }} />
                <ReferenceLine x={result.p95} stroke="#f43f5e" strokeDasharray="3 3" label={{ value: 'p95', fill: '#f43f5e', fontSize: 10 }} />
